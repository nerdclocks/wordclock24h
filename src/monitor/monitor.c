/*-------------------------------------------------------------------------------------------------------------------------------------------
 * monitor.c - wclock12/24 monitor (mcurses)
 *
 ...
 */
void
monitor_show_temperature_ds (uint_fast8_t temperature_index)
{
    if (mcurses_is_up)
    {
        move (DS18XX_TEMP_LINE, DS18XX_TEMP_COL);
        monitor_show_temperature (temperature_index);
    }
}

/*-------------------------------------------------------------------------------------------------------------------------------------------
 * print temperature of RTC
 *
 *   index ==   0  ->   0°C
 *   index == 250  -> 125°C
 *-------------------------------------------------------------------------------------------------------------------------------------------
 */
void
monitor_show_temperature_rtc (uint_fast8_t temperature_index)
{
    if (mcurses_is_up)
    {
        move (RTC_TEMP_LINE, RTC_TEMP_COL);
        monitor_show_temperature (temperature_index);
    }
}
...
#if WCLOCK24H == 1
void
monitor_show_temperature_on_display (uint_fast8_t temperature_index)
{
